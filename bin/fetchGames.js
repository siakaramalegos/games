...
function hasValidPollResults(playerPollResults) {
  ...
  const invalid =
    first.best == 0 && first.recommended == 0 && first.notRecommended == 0;
  return !invalid;
}
...
async function mungeGames(
  ...
  const updatedCache = cachedGames.filter(
    (game) => !deletedIds.has(game.gameId)
  );
  ...
  const addedBaseGames = currentBasicGamesWithComplexities.filter(
    (game) => !game.isExpansion && addedIds.has(game.gameId)
  );
  ...
  const myExpansionIds = currentBasicGamesWithComplexities
    ...
    if (expansions) {
      ...
      if (myBasicExpansions.length > 0) {
        modifiedGame.myExpansions = myBasicExpansions.map((expansion) => {
          return currentBasicGamesWithComplexities.find(
            (element) => element.gameId === expansion.gameId
          );
        });
      }
    }
  ...
  const complexities = await Promise.all(
    addedIds.map(async (id) => {
      ...
      const complexity = await page.evaluate(() => {
        return document.querySelector("[item-poll-button=boardgameweight] span")
          .innerText;
      });
...
async function getComplexities(cachedComplexities, currentBasicGames) {
  ...
  return [...cachedComplexities, ...addedRatings];
}

async function getGames() {
  console.log(">>> Loading cached games...");
  const cachedGames = readFromCache("src/_data/games.json");
  ...
  const cachedComplexities = readFromCache("bin/complexities.json");
  const newComplexities = await getComplexities(
    cachedComplexities,
    currentBasicGames
  );
  writeToFile("bin/complexities.json", newComplexities);

  // Can ignore expansions since no additional requests will be needed to populate them
  const { addedIds, deletedIds } = checkDifferences(
    cachedGames,
    currentBasicGames.filter((game) => !game.isExpansion)
  );
  console.log({ addedIds, deletedIds });

  const mungedGames = await mungeGames(
    cachedGames,
    currentBasicGames,
    addedIds,
    deletedIds,
  ...
  writeToFile("src/_data/games.json", mungedGames);
